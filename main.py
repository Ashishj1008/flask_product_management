...
def db_connection():
    conn = None
    try:
        conn = sqlite3.connect("product_details_explain.db")
...
def data_validations(data):
    if not data[0] or len(data[0]) <= 1:
        ...
        conn = db_connection()
        cursor = conn.cursor()
        sql = """INSERT INTO product_described(product_name, price, quantity,created_at,is_active) VALUES(?,?,?,
                        ?,?) """
...
def get_products():
    try:
        conn = db_connection()
        cursor = conn.execute("SELECT * FROM product_described")
        product_dic = {row[0]: {"product_name": row[1], "price": row[2], "quantity": row[3], "created_at": row[4],
                                "is_active": row[5]}
...
def get_product_details(id):
    try:
        conn = db_connection()
        cursor = conn.execute(f"SELECT * FROM product_described WHERE id=?", (id,))
        product_dic = {row[0]: {"product_name": row[1], "price": row[2], "quantity": row[3], "created_at": row[4],
                                "is_active": row[5]}
                       for row in cursor.fetchall()}
        if product_dic:
...
def products_greater(price1, price2):
    try:
        conn = db_connection()
        cursor = conn.execute(f"SELECT * FROM product_described WHERE price BETWEEN {price1} AND {price2}")
        product_dic = {row[0]: {"product_name": row[1], "price": row[2], "quantity": row[3], "created_at": row[4],
                                "is_active": row[5]}
                       for row in cursor.fetchall()}
        if product_dic:
...
def create_product_item():
    try:
        ...
        data = [product_name, price, quantity, created_at, is_active]
...
def updated_product_details(id):
    try:
        ...
        conn = db_connection()
        cursor = conn.cursor()
        sql = "UPDATE product_described SET product_name=?, quantity=?, price=? WHERE id=? "
        updated_table = {
            "id": id,
            "price": price,
...
def deleted_values(id):
    try:
        conn = db_connection()
        sql = f"SELECT * FROM product_described WHERE id={id}"
        cursor = conn.execute(sql)

        if cursor.fetchall():
            sql = """ DELETE FROM product_described WHERE id=? """
            conn.execute(sql, (id,))
            conn.commit()
